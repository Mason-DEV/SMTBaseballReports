...
async function ExecuteAuditEmail(ffxAudit) {
    ...
    var supportPDF = null;
    await Promise.all([
        axios.post(`${process.env.HOST}:${process.env.PORT}/api/auditPdfBuilder/buildOpAuditPDF` , ffxAudit, { headers: { Authorization: devToken }}),
        axios.post(`${process.env.HOST}:${process.env.PORT}/api/auditPdfBuilder/buildSupportAuditPDF` , ffxAudit, { headers: { Authorization: devToken }})
        ])
